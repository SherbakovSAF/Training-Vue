   ...
   <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          ...
          <div class="container">
               <section>
                    <div class="flex">
                         <div class="max-w-xs">
                              ...
                              <div class="mt-1 relative rounded-md shadow-md">
                                   <input v-model="inputTicker" v-on:focus="removeAlertMessage()"
                                        v-on:keydown.enter="addTicker()" type="text" name="wallet" id="wallet"
                                        class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                                        placeholder="Например DOGE" />
                              </div>
                              <div v-if="renderTemplateInput().length > 0 && inputTicker.length"
                                   class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
                                   <span v-for="(tickerName, idx) in renderTemplateInput().slice(0, 4)"
                                        @click="addTemplateTickerFromInput(tickerName)" v-bind:key="idx"
                    ...
                    <button type="button" v-on:click="addTicker()"
               ...
               <div>
                    <input type="text" placeholder="Введите имя для фильтрации" v-model="filterInput">
                    <button type="button"
                         class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
                         @click="page = page - 1" v-if="page > 1">
                         Назад
                    </button>
                    <button type="button"
                         class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
               ...
               <template v-if="tickerState.length">
                    ...
                    <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
                         <div v-for="(ticker, idx) in paginatedTicker" v-bind:key="idx" 
                              @click="updateGraph(ticker)"
                              v-bind:class="checkedTicker == ticker ? 'border-4' : ''"
                              class="bg-white overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer">
                              <div class="px-4 py-5 sm:p-6 text-center" :class="{
                                   'bg-red-100': formatPrice(ticker.price) == '-',
                              }">
                              ...
                              <button v-on:click.stop="removeTicker(ticker)"
     ...
     methods: {
          ...
          addTicker() {
               let newTicker = {
                    ...
                    name: this.inputTicker.toUpperCase(),
                    price: "-",
                    graphTicker: []
               }
